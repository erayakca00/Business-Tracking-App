...
const TOOLTIP_STYLE = { backgroundColor: 'rgba(31, 41, 55, 0.95)', border: 'none', borderRadius: '8px', color: '#fff', fontSize: '12px' };
...
function formatDateTick(str: string): string {
    try {
        ...
        console.warn('Failed to parse date for axis tick:', err);
        return str;
    }
}

/* ── Burndown Chart ── */
function BurndownChartSection({ burndown, burndownMetric, setBurndownMetric }: Readonly<{
    burndown: any;
    burndownMetric: 'tasks' | 'effort';
    setBurndownMetric: (v: 'tasks' | 'effort') => void;
}>) {
    const hasData = burndown?.burndownData?.length > 0;
...
/* ── Status Pie Chart ── */
function StatusPieSection({ pieData }: Readonly<{ pieData: { name: string; value: number; fill: string }[] }>) {
    return (
                        ...
                        <div className="grid grid-cols-2 gap-x-4 gap-y-2 mt-4 text-xs font-medium w-full">
                            {pieData.map((entry) => (
                                <div key={entry.name} className="flex items-center space-x-2">
                                    <span className="h-3 w-3 rounded-full flex-shrink-0" style={{ backgroundColor: entry.fill }}></span>
...
/* ── Overdue Tasks Table ── */
function OverdueTableSection({ overdueTasks, groupId }: Readonly<{ overdueTasks: any[]; groupId: string | undefined }>) {
    return (
...
function computeSummaryMetrics(statusBreakdown: Record<string, number>) {
    const totalTasks = Object.values(statusBreakdown || {}).reduce((a: number, b: number) => a + b, 0);
    const completedTasksCount = statusBreakdown?.done || 0;
    const completionRate = totalTasks > 0 ? Math.round((completedTasksCount / totalTasks) * 100) : 0;
    return { totalTasks, completedTasksCount, completionRate };
}

function computePieData(statusBreakdown: Record<string, number>) {
    return Object.entries(statusBreakdown || {})
        .filter(([_, value]) => value > 0)
        .map(([key, value]) => ({
            name: STATUS_LABELS[key] || key,
            value: value,
            fill: STATUS_COLORS[key] || '#9CA3AF'
        }));
}
...
function computeTimeTrackingData(timeReport: TimeLog[]) {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 flex items-center justify-between transition-colors">
                <div>
                    ...
                    <h3 className="text-3xl font-extrabold mt-1 text-rose-500">{overdueCount}</h3>
...
function SprintVelocitySection({ sprintVelocity }: Readonly<{ sprintVelocity: any[] }>) {
    return (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col transition-colors">
            <h2 className="text-lg font-bold mb-1 flex items-center gap-2">
            ...
            <div className="flex-1 min-h-[250px] flex items-center justify-center">
                {sprintVelocity && sprintVelocity.length > 0 ? (
...
function MemberWorkloadSection({ memberWorkload }: Readonly<{ memberWorkload: any[] }>) {
    return (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col transition-colors">
            <h2 className="text-lg font-bold mb-1 flex items-center gap-2">
            ...
            <div className="flex-1 min-h-[250px] flex items-center justify-center">
                {memberWorkload && memberWorkload.length > 0 ? (
                    <div className="w-full h-full min-h-[250px]">
                        <ResponsiveContainer width="100%" height={250}>
                            <BarChart
                                data={memberWorkload}
...
/* ── Member Time Section ── */
function MemberTimeSection({ memberTimeData }: Readonly<{ memberTimeData: any[] }>) {
    return (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col transition-colors">
            <h2 className="text-lg font-bold mb-1 flex items-center gap-2">
...
/* ── Task Time Section ── */
function TaskTimeSection({ taskTimeData }: Readonly<{ taskTimeData: any[] }>) {
    return (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col transition-colors">
            <h2 className="text-lg font-bold mb-1 flex items-center gap-2">
...
export default function Analytics() {
    ...
    // Pie chart mapping — include `fill` so Recharts colours each slice without the deprecated Cell
    const pieData = computePieData(analytics.statusBreakdown);

    // Summary calculations
    const { totalTasks, completedTasksCount, completionRate } = computeSummaryMetrics(analytics.statusBreakdown);

    // Aggregations for Time Logs
    const { totalTimeLoggedSeconds, memberTimeData, taskTimeData } = computeTimeTrackingData(timeReport);
        ...
        <div className="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 transition-colors duration-200 flex flex-col pb-12">
            ...
            <main className="max-w-[96%] w-full mx-auto px-4 mt-8 flex-1 flex flex-col space-y-8">
                ...
                <MetricSummaryCards
                    ...
                    overdueCount={analytics.overdueTasks?.length || 0}
                    totalHoursLogged={Math.round((totalTimeLoggedSeconds / 3600) * 10) / 10}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <StatusPieSection pieData={pieData} />
                </div>

                {/* Secondary Row: Velocity & Workload */}
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    {/* Velocity Chart */}
                    <SprintVelocitySection sprintVelocity={analytics.sprintVelocity} />

                    {/* Member Workload */}
                    <MemberWorkloadSection memberWorkload={analytics.memberWorkload} />
                </div>

                {/* Time Tracking Analysis Row */}
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    {/* Time Logged per Member */}
                    <MemberTimeSection memberTimeData={memberTimeData} />

                    {/* Time Logged per Task */}
                    <TaskTimeSection taskTimeData={taskTimeData} />
                </div>

                {/* Overdue Tasks List */}
                <OverdueTableSection overdueTasks={analytics.overdueTasks || []} groupId={groupId} />
            </main>
        </div>
    );
}
